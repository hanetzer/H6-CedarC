...
void ion_alloc_flush_cache(void* startAddr, int size)
{
    ...
    int flag = 0;
    unsigned long vir = (unsigned long)startAddr;
    buffer_node * tmp;
    ion_fd_data_t fd_data;
    aw_mem_list_for_each_entry(tmp, &g_alloc_context->list, i_list)
    {
        if (vir >= tmp->vir
        && vir < tmp->vir + tmp->size)
        {
            fd_data.aw_fd = tmp->fd_data.aw_fd;
            flag = 1;
            break;
        }
    }
    if (0 == flag)
    {
        loge("ion_alloc_flush_cache get vir_addr:%lx's buf_fd fail\n", vir);
        pthread_mutex_unlock(&g_mutex_alloc);
        return;
    }

    ret = ioctl(g_alloc_context->fd, AW_MEM_ION_IOC_SYNC, &fd_data);
    if (ret)
    {
        loge("AW_MEM_ION_IOC_SYNC failed, ret is %d \n", ret);
    }
...
#elif (defined CONF_KERNEL_VERSION_4_9)
void ion_alloc_flush_cache(void* startAddr, int size)
{
    int ret;
    sunxi_cache_range range;
